        ...
        while (countTable(nearestBlips) > 0) do
            ...
            for index, dist in pairs(_cache) do
                if (dist <= 10 and not nui) then
					...
					DrawMarker(36, config.coord.x, config.coord.y, config.coord.z+0.2, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 0.8, 0, 235, 216, 52, 1, 0, 0, 1)
					DrawMarker(27, config.coord.x, config.coord.y, config.coord.z-0.5, 0, 0, 0, 0, 0, 0, 3.0, 3.0, 3.0, 0, 235, 216, 52, 0, 0, 0, 1)
...
openBennys = function(config)
	if (vSERVER.checkPermission(config.perm)) then
		...
		if (vehicle and vSERVER.checkVehicle(VehToNet(vehicle))) then
			...
			TriggerEvent('notify', 'quantum Mecânica', 'Você precisa estar dentro de um veículo!')
		end
	else
		TriggerEvent('notify', 'quantum Mecânica', 'Você não possui permissão!')
	end
end
...
isVehicleTooFar = function(veh)
	Citizen.CreateThread(function()
		while (nui) do
			...
			if (distance > 7) then
				closeNui()
				TriggerEvent('Notify', 'quantum Mecânica', 'Você se afastou muito do <b>veículo</b>.')
			end
			Citizen.Wait(500)
		end
	end)
...
RegisterNetEvent('quantum_bennys:applymods')
AddEventHandler('quantum_bennys:applymods', function(veh, custom)
	if (veh) then
		TriggerServerEvent('quantum_bennys:syncApplyMods', custom, veh)
	end
end)

RegisterNetEvent('quantum_bennys:applymods_sync')
AddEventHandler('quantum_bennys:applymods_sync',function(custom, vnet)
...
RegisterNUICallback('pagar',function(data)
	if (cart['total'] and vSERVER.checkPayment(parseInt(cart['total']))) then
		...
		Citizen.SetTimeout(_time, function() 
			...
			_, _, vehplate, vehname = quantum.vehList(7)
			...
			if (vSERVER.saveVehicle(vehplate, myVehicle)) then
				TriggerEvent('notify', 'quantum Mecânica', 'Modificações aplicadas com sucesso. Você pagou <b>R$'..cart['total']..'<b>.')
				closeNui()
			else
				vSERVER.givePayment(cart['total'])
				TriggerEvent('notify', 'quantum Mecânica', 'A placa deste <b>veículo</b> não existe, tente puxa-lô novamente mais tarde. O carro não terá o tuning salvo na garagem.')
				closeNui()
			end
		end)
	end
