...
RegisterNetEvent('quantum_doors:statusSend')
AddEventHandler('quantum_doors:statusSend',function(i,status)
            ...
            for index, dist in pairs(_cache) do
                ...
                if (dist <= (open or 1.8)) then
                    ...
                    if (dist <= (open or 1.2) and IsControlJustPressed(0, 38) and vSERVER.checkPermissions(_config.perm, _config.home) and GetEntityHealth(ped) > 100) then
                        if (open) then if (not IsPedInAnyVehicle(ped)) then quantum._playAnim(true, {{ 'anim@mp_player_intmenu@key_fob@', 'fob_click' }}, false); end; end;
                        TriggerServerEvent('quantum_doors:open', index, _config.autoLock)
                    end
                end
            end
            Citizen.Wait(1)
