...
quantum.prepare('quantum_dismantle/getVehicleInfo', 'select * from '..table..' where user_id = @user_id and vehicle = @vehicle')
...
srv.checkDismantle = function(index)
    ...
    if (user_id) and quantum.checkPermissions(user_id, _config.perm) then
        ...
        local vehicle, vnetid = quantumClient.vehList(source, 3.0)
        if (vehicle) then
            ...
            if (vehState) then
                ...
                local vtype = exports.quantum_garage:vehicleType(vehState.model)
                if (vtype) then
                    registerDismantleTime(index, _config.cooldown)
                    vCLIENT.generateParts(source, vehicle, vtype, vehState.model, vehState.plate)
                    quantum.execute('quantum_bennys/setCustom', { user_id = vehState.user_id, plate = vehState.plate, custom = json.encode({}) })
                    quantum.execute('quantum_garage/setDetained', { detained = 1, user_id = vehState.user_id, vehicle = vehState.model })
                    quantum.webhook(_config.webhook, '```prolog\n[DISMANTLE]\n[ACTION]: (DISMANTLED VEHICLE)\n[USER]: '..user_id..' \n[VEHICLE MODEL]: '..vehState.model..'\n[OWNER VEHICLE]: '..vehState.user_id..' \n[COORDS]: '..tostring(GetEntityCoords(GetPlayerPed(source)))..' '..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                    return true
                end
            end
        end
...
srv.finishDismantle = function(vtype)
    ...
    if (user_id) then
        ...
        if (price) then
            quantum.giveInventoryItem(user_id, Dismantle.payments['item'], Dismantle.payments[price])
            TriggerClientEvent('notify', source, 'Desmanche', 'Você recebeu <b>R$'..quantum.format(Dismantle.payments[price])..'</b> de dinheiro sujo')
        end
    end
end
