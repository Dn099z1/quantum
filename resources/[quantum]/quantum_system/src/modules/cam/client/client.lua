...
camMenu = NativeUI.CreateMenu('quantum Cam', 'Controle a Câmera Cinemática')
...
local CamThread = function()
    Citizen.CreateThread(function()
        ...
        TriggerServerEvent('quantum_cam:disableCam')
    end)
...
function ProcessCamControls()
    ...
    if (isAttached) then
        ...
        if (Vdist(0.0, 0.0, 0.0, offsetCoords.x, offsetCoords.y, offsetCoords.z) > 40.0) then
            TriggerEvent('notify', 'quantum Cam', 'Você se <b>afastou</b> do seu personagem!')
            AttachCamToEntity(cam, entity, offsetCoords.x, offsetCoords.y, offsetCoords.z, true)
        end
