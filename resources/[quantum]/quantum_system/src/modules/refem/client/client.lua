...
cli.takeHostage = function(targetSrc)
    ...
    if (not Weapons[GetSelectedPedWeapon(ply)]) then
        TriggerEvent('notify', 'Refem', 'Você não está segurando nenhuma <b>pistola</b>.')
        return false
    end

    if (GetAmmoInPedWeapon(ply, GetHashKey(Weapons[GetSelectedPedWeapon(ply)])) <= 0) then
        TriggerEvent('notify', 'Refem', 'Você não possui munição em seu <b>armamento</b>.')
        return false
    end
...
RegisterNetEvent('quantum_prefem:syncTarget')
AddEventHandler('quantum_prefem:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
...
RegisterNetEvent('quantum_prefem:syncMe')
AddEventHandler('quantum_prefem:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
...
RegisterNetEvent('quantum_prefem:cl_stop')
AddEventHandler('quantum_prefem:cl_stop', function()
    local ply = PlayerPedId()
        ...
        Citizen.CreateThread(function()
            ...
            while (LocalPlayer.state.holdingHostage) do
                ...
                if (GetEntityHealth(ply) <= 100) then 
                    TriggerServerEvent('quantum_prefem:stop', target)
                    Citizen.Wait(100)
                    releaseHostage()
                    LocalPlayer.state.holdingHostage = false
                end
...
releaseHostage = function()
	...
	if (closestPlayer ~= nil) then
		TriggerServerEvent('quantum_prefem:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag)
	end
end 
...
killHostage = function()
	...
	if (closestPlayer ~= nil) then
		TriggerServerEvent('quantum_prefem:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag)
        TriggerServerEvent('quantum_prefem:killHostage')
    end	
end
