quantum.teleport = function(x, y, z)
...
quantum.getNearestPlayers = function(radius)
	local r = {}
	local pid = PlayerId()
...
quantum.getNearestPlayer = function(radius)
	local p = nil
	local players = quantum.getNearestPlayers(radius)
...
quantum.playAnim = function(upper, seq, looping)
	if seq.task then
		quantum.stopAnim(true)

		local ped = PlayerPedId()
		if seq.task == 'PROP_HUMAN_SEAT_CHAIR_MP_PLAYER' then
			local x,y,z = quantum.getPosition()
			TaskStartScenarioAtPosition(ped,seq.task,x,y,z-1,GetEntityHeading(ped),0,0,false)
		else
			TaskStartScenarioInPlace(ped,seq.task,0,not seq.play_exit)
		end
	else
		quantum.stopAnim(upper)
...
quantum.stopAnim = function(upper)
	anims = {}
	if upper then
...
quantum.playSound = function(dict, name)
	PlaySoundFrontend(-1, dict, name, false)
end

quantum.playScreenEffect = function(name, duration)
	if duration < 0 then
...
Citizen.CreateThread(function()
	while (NetworkIsSessionStarted()) do
		TriggerServerEvent('dx:joinedInServer')
		break
		Citizen.Wait(30000)
	end
end)
...
AddEventHandler('playerSpawned', function()
	TriggerServerEvent('quantumClient:playerSpawned')
end)