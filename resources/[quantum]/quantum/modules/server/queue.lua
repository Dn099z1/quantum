...
------------------------------------------------------------------
-- PEGAR OS IDS QUE ESTÃO NA FILA
------------------------------------------------------------------
...
function Queue:AddToConnecting(ids,ignorePos,autoRemove,done, name)
	local function removeFromQueue()
		if not autoRemove then return end;

		done(language.connectingError)
...
local function playerConnect(name, _, deferrals)
	local src = source
...
local function checkTimeOuts()
	...
	while i <= Queue:GetSize() do
		...
		if not data.ids or not data.name or not data.firstconnect or data.priority == nil or not data.source then
			data.deferrals.done(language.desconnect)
...
------------------------------------------------------------------
AddEventHandler('checkTimeOuts', checkTimeOuts)
AddEventHandler('playerConnecting', playerConnect)

RegisterNetEvent('dx:joinedInServer')
AddEventHandler('dx:joinedInServer', function()
	local _source = source
	if (_source) then
		TriggerEvent('queue:playerConnectingRemoveQueues', Queue:GetIds(_source))
	end
end)
...
------------------------------------------------------------------
-- COMANDO PARA SABER QUANTAS PESSOAS ESTÃO NA FILA
------------------------------------------------------------------
RegisterCommand('fila', function(source)
	if source == 0 then 
		print('^5[FILA DO SERVIDOR]:^7 '..Queue:GetSize()..' jogadores.')
	else
		if (quantum.hasPermission(quantum.getUserId(source), 'staff.permissao')) then
