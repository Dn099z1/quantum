        ...
        while (install) do
            ...
            if (pVehicle and not IsPedInAnyVehicle(ped)) then
                ...
                if (distance <= 5.0) then
                    idle = 1
                    DrawMarker(0, engine.x, engine.y, engine.z+0.7, 0, 0, 0, 0, 0, 0, 0.3, 0.3, 0.3, 216, 179, 13, 155, 1, 0, 0, 1)
                    if (distance <= 1.5 and IsControlJustPressed(0, 38) and vSERVER.getNitro()) then
                        installing = true
                        quantum._playAnim(false, {{ 'mini@repair', 'fixing_a_player' }}, true)
                        ...
                        Citizen.SetTimeout(5000, function()
                            ...
                            TriggerServerEvent('quantum_tunings:syncNitro', VehToNet(pVehicle), 100.0)
                            TriggerServerEvent('quantum_tunings:syncPurge', VehToNet(pVehicle), 0.0)
        ...
        while (thread) do
            ...
            if (IsPedInAnyVehicle(ped) and GetEntityHealth(ped) > 100) then
                if (veh and hasNitro(veh)) then
                    ...
                    if (nosLevel < 1) then
                        TriggerServerEvent('quantum_tunings:syncNitro', VehToNet(veh), false)
                    ...
                    if (activated and nosLevel > 0) then
                        ...
                        TriggerServerEvent('quantum_tunings:syncNitro', VehToNet(veh), lvl)
                        SendNUIMessage({ type = 'nosLevel', nos = lvl })

                        if (purgeLevel < 100) then
                            local lvl = purgeLevel + 4.0
                            TriggerServerEvent('quantum_tunings:syncPurge', VehToNet(veh), lvl)
