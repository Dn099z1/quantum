...
srv.getNitro = function()
    ...
    if (user_id) then
        if (quantum.tryGetInventoryItem(user_id, 'nitro', 1)) then
            return true
        end
...
RegisterNetEvent('quantum_tunings:syncNitro', function(vehicle, value)
    GlobalNitro[vehicle] = value
end)

RegisterNetEvent('quantum_tunings:syncPurge', function(vehicle, value)
...
startInstallNitro = function(source)
    local user_id = quantum.getUserId(source)
    if (quantum.getInventoryItemAmount(user_id, 'nitro') > 0) then
