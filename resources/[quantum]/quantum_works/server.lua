...
srv.checkItem = function(_require, production)
    ...
    if (_userId) then
        if (production) then
            if (_require) then
                for index, value in pairs(_require) do
                    ...
                    if quantum.getInventoryWeight(_userId) + quantum.getItemWeight(index) *quantity <= quantum.getInventoryMaxWeight(_userId) then
                        if (quantum.getInventoryItemAmount(_userId, index) >= quantity) then
                            if quantum.tryGetInventoryItem(_userId, index, quantity) then
                                return true
                            end
                        end
                        TriggerClientEvent('notify', _source, 'Emprego', 'Você não possui <b>'..quantity..'x</b> de <b>'..quantum.itemNameList(index)..'</b>.')
                        return false
                    end
        ...
        elseif (_require) then
            if quantum.getInventoryWeight(_userId) + quantum.getItemWeight(_require['spawn']) * _require['quantity'] <= quantum.getInventoryMaxWeight(_userId) then
                if (quantum.getInventoryItemAmount(_userId, _require['spawn']) >= _require['quantity']) then
                    if quantum.tryGetInventoryItem(_userId, _require['spawn'], _require['quantity']) then
                        return true
                    end
                end
                TriggerClientEvent('notify', _source, 'Emprego', 'Você não possui <b>'.._require['quantity']..'x</b> de <b>'..quantum.itemNameList(_require['spawn'])..'</b>.')
                return false
            end
            TriggerClientEvent('notify', _source, 'Emprego', 'Você não possui <b>espaço</b> na mochila.')
            return false
    ...
    if (_userId) then
        if (#(GetEntityCoords(GetPlayerPed(_source)) - location[selection[_userId]]) <= 5.0) then
            ...
            if (selection[_userId] >= _payment['bonus']['after']) then
                ...
                quantum.giveMoney(_userId, parseInt(randomMoney * bonus))
            ...
            exports.quantum_bank:extrato(_userId, work, parseInt(randomMoney * bonus))
            ...
            print('^5[quantum Anti]^7 o usuário '.._userId..' está provavelmente tentando dumpar!')
        end
    end
    return false
end
        ...
        if (_userId) then
            if (product[config] and not active[_userId]) then
                ...
                for index, value in pairs(_receive) do
                    local quantity = math.random(value['min'], value['max'])
                    if quantum.getInventoryWeight(_userId) + quantum.getItemWeight(index) * quantity <= quantum.getInventoryMaxWeight(_userId) then
                        quantum.giveInventoryItem(_userId, index, quantity)
                        TriggerClientEvent('notify', _source, 'Emprego', 'Você recebeu <b>'..quantity..'x</b> de <b>'..quantum.itemNameList(index)..'</b>.')
