    ...
    for k,v in pairs(config.chests) do
        ...
        if nextToChest then
            ...
            if IsControlJustPressed(0,38) then
                ...
                if not quantum.getNearestPlayer(config.marker_dropped_item_distance) or isPrivate then
...
hotbarListener = function()
    if GetEntityHealth(PlayerPedId()) <= 100 then
        ...
        exports.quantum_inventory:closeInventory()
    end
end

trunkDistanceControl = function()
    if nearbyVehicle ~= nil then 
        local vehicle = quantum.vehList(5)
