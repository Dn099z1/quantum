...
cInventory.getChestInfo = function(chestType)
...
RegisterCommand("openCarChest", function()
    if quantum.getNearestPlayer(config.marker_dropped_item_distance + 1) ~= nil then
    ...
    if GetEntityHealth(ped) > 100 and not config.functions.isHandcuffed() and not IsPedBeingStunned(ped) and not IsPlayerFreeAiming(ped) then
        local vehicle,vnetid,placa,vname,lock,banned,trunk  = quantum.vehList(5)
