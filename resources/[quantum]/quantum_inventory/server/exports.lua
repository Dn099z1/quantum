sInventory.giveBagItem = function(bagType, item, amount)
    ...
    if config.items[item] then
        local slots = nil
        local items = quantum.query('quantum_inventory:getBag', { bag_type = bagType })
        ...
        if slots ~= nil then
            ...
            quantum.execute('quantum_inventory:insertBag', { slots = json.encode(updatedSlots), bag_type = bagType, weight = currentWeight })
            return true
        end
    else
...
sInventory.giveInventoryItem = function(user_id, item, amount)
    -- TriggerClientEvent('notify', quantum.getUserSource(user_id), 'Inventário', 'Você recebeu <b>'..amount..'x</b> de <b>'..quantum.itemNameList(item)..'</b>.')
    local _source = source
    
    if user_id == 0 then
        user_id = quantum.getUserId(_source)
    end
    ...
    local ok, fail = pcall(function()       
        ...
        if config.items[item] then
            local slots = nil
            local items = quantum.query('quantum_inventory:getBag', { bag_type = 'bag:'..user_id })
            if #items > 0 then slots = json.decode(items[1].slots) end
            local updatedSlots = {}
            ...
            if slots ~= nil then
                ...
                updatedSlots[0] = { index = item, amount = amount }
                quantum.execute('quantum_inventory:insertBag', { slots = json.encode(updatedSlots), bag_type = 'bag:'..user_id, weight = config.bag_max_weight })
            end
        else
            config.functions.serverNotify(_source, config.texts.notify_title, config.texts.notify_non_existent_item)
        end
...
sInventory.tryGetInventoryItem = function(user_id, index, amount)
    -- TriggerClientEvent('notify', quantum.getUserSource(user_id), 'Inventário', 'Você perdeu <b>'..amount..'x</b> de <b>'..quantum.itemNameList(index)..'</b>.')
    index = string.lower(index)
    
    local items = quantum.query('quantum_inventory:getBag', { bag_type = 'bag:'..user_id })
    local hotbarItems = quantum.query('quantum_inventory:getBag', { bag_type = 'hotbar:'..user_id })
    local slots = {}
    local hotbarSlots = {}
    if #items > 0 then slots = json.decode(items[1].slots) end
    ...
    quantum.execute('quantum_inventory:updateBag', { slots = json.encode(slots), bag_type = 'bag:'..user_id })
    quantum.execute('quantum_inventory:updateBag', { slots = json.encode(hotbarSlots), bag_type = 'hotbar:'..user_id })
...
sInventory.getInventoryWeight = function(user_id)
    local bag = quantum.query('quantum_inventory:getBag', { bag_type = 'bag:'..user_id })
    local hotbar = quantum.query('quantum_inventory:getBag', { bag_type = 'hotbar:'..user_id })
...
sInventory.getInventoryMaxWeight = function(user_id)
    local bag = quantum.query('quantum_inventory:getBag', { bag_type = 'bag:'..user_id })[1] or { weight = config.bag_max_weight }
    return bag.weight
end
exports('getInventoryMaxWeight', sInventory.getInventoryMaxWeight)

sInventory.setInventoryMaxWeight = function(user_id, weight)
    quantum.query('quantum_inventory:updateBagWeight', { weight = weight, bag_type = 'bag:'..user_id })
end
...
sInventory.clearInventory = function(user_id)
    ...
    quantum.execute('quantum_inventory:deleteBag', { bag_type = 'bag:'..user_id })
    quantum.execute('quantum_inventory:deleteBag', { bag_type = 'hotbar:'..user_id })
end
...
AddEventHandler('quantum:playerSpawn', function(user_id, source)    
    ...
    if (not bag) then
        quantum.execute('quantum_inventory:insertBag', { slots = json.encode({}), bag_type = 'bag:'..user_id, weight = 6 })
