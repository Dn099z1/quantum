sInventory.getBag = function(bag_type)
    local items = {}    
    items = quantum.query('quantum_inventory:getBag', { bag_type = bag_type })
    if #items > 0 then
...
sInventory.getSlotsByBag = function(bag_type)
    return quantum.query('quantum_inventory:getBag', { bag_type = bag_type })[1].slots or {}
end
exports('getSlotsByBag', sInventory.getSlotsByBag)

sInventory.modifyBag = function(bagType, newBag)
    quantum.execute('quantum_inventory:updateBagWithoutId', { slots = json.encode(newBag), bag_type = bagType })
end
