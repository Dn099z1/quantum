...
srv.checkPermissions = function(permission)    
    ...
    local _userId = getUserId(_source)
    if (_userId) then
        return quantum.checkPermissions(_userId, permission)
    end
end
...
srv.giveWeapon = function(weapons, _config)
    ...
    if (_userId) then
        ...
        if (not cooldownWeapons[_userId][_spawn]) then
            if (not exports['quantum_arsenal']:checkPermissions(_config['weapons']['cooldownWeapons']['noCooldown'])) then 
                cooldownWeaponsThread(_userId, _spawn, _config['weapons']['cooldownWeapons']['time']) 
            end

            if (not _weapons[_spawn]) then
                if (quantum.tryFullPayment(_userId, weapons['price'])) then
                    ...
                    TriggerClientEvent('notifyArsenal', _source, 'Você não possui <b>R$'..quantum.format(weapons['price'])..'</b>!')
                end
            else
...
srv.giveUtilitary = function(utilitarys, _config)
    ...
    if (_userId) then
        ...
        if (not cooldownUtilitarys[_userId][_spawn]) then
            if (not exports['quantum_arsenal']:checkPermissions(_config['utilitarys']['cooldownUtilitarys']['noCooldown'])) then 
            ...
            if (utilitarys['type'] == 'arma') then
                local _weapons = getWeapons(_source)
                if (not _weapons[_spawn]) then
                    if (quantum.tryFullPayment(_userId, utilitarys.price)) then
                        ...
                        TriggerClientEvent('notifyArsenal', _source, 'Você não possui <b>R$'..quantum.format(utilitarys.price)..'</b>!')
                    end
                else
...
srv.giveKits = function(kits, _config)
    ...
    if (_userId) then
        ...
        if (not cooldownKits[_userId][_name]) then
            if (not exports['quantum_arsenal']:checkPermissions(_config['kits']['cooldownKits']['noCooldown'])) then 
            ...
            for index, value in pairs(kits['itens']) do
                if (getInventoryItemAmount(_userId, index) < value['quantity']) then
                    if (quantum.tryFullPayment(_userId, value.price)) then
                        giveInventoryItem(_userId, index, value['quantity'])
                    else
                        TriggerClientEvent('notifyArsenal', _source, 'Você não possui <b>R$'..quantum.format(value.price)..'</b>!')
                    end
                else
