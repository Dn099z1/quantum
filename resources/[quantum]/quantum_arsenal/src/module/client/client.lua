...
local createNPC = function()
    ...
    for index, value in pairs(locsConfig) do
        ...
        entity = CreatePed(4, value['ped']['skin'], value['ped']['coord']['x'], value['ped']['coord']['y'], value['ped']['coord']['z']-1, value['ped']['coord']['w'], false, true)
    ...
    while true do
        ...
        if not (inArsenal and not nui) then
            if (nearestBlip) and nearestBlip['coord'] then
                ...
                if (nearestBlip['close'] and IsControlJustPressed(0, 38) and GetEntityHealth(ped) > 101 and not IsPedInAnyVehicle(ped) and vSERVER.checkPermissions(nearestBlip.perm)) then
                    openArsenal()
                end
            end
        end
