...
quantum.prepare('quantum_identity/getUserRH', 'select rh from creation where user_id = @user_id')
getUserRH = function(user_id)
    local query = quantum.query('quantum_identity/getUserRH', { user_id = user_id })[1]
    if (query) then
        return query.rh
    end
    return 'Desconhecido'
