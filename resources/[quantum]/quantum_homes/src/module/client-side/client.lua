...
cli.enterHome = function(interior, decorations, name)
    ...
    DoScreenFadeOut(100)
    TriggerEvent('quantum_sound:source', 'enterexithouse', 0.7)
    Citizen.Wait(500)
...
exitHome = function()
    ...
    DoScreenFadeOut(100)
	Citizen.Wait(500)
    TriggerEvent('quantum_sound:source', 'enterexithouse', 0.5)
            ...
            if (inHome) then
                for k, v in pairs(tmpHomes.internLocates) do
                    ...
                    if (distance <= 5.0) then
                        idle = 1
                        DrawMarker(0, coord.x, coord.y, coord.z, 0, 0, 0, 0, 0, 0, 0.3, 0.3, 0.3, 0, 235, 216, 52, 1, 0, 0, 1)
                        if (distance <= 1.2 and IsControlJustPressed(0, 38) and GetEntityHealth(ped) > 100) then
                            if (v[4] == 'exit') then
                                exitHome()
                            elseif (v[4] == 'vault') then
                                if (vSERVER.vaultPermissions(tmpHomes.homeName)) then  exports['quantum_inventory']:openInventory('open', 'homes:'..tmpHomes.homeName); end;
                            end
                        end
                    end
                end
...
RegisterNetEvent('quantum_interactions:blips', function()
    if (homeList) then
        homeList = false
...
cli.createGarage = function(homeCoords)
    ...
    local stages = {
        [1] = function(ped, pCoord)
            ...
            DrawMarker(36, pCoord.x, pCoord.y, pCoord.z, 0, 0, 0, 0, 0, 0, 0.7, 0.7, 0.7, 0, 235, 216, 52, 1, 0, 0, 1)
            DrawMarker(27, pCoord.x, pCoord.y, pCoord.z-0.97, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 0, 235, 216, 52, 0, 0, 0, 1)
        ...
        [2] = function(ped, pCoord)
            ...
            DrawMarker(26, pCoord.x, pCoord.y, pCoord.z, 0, 0, 0, pRotation.x, pRotation.y, pRotation.z, 1.0, 1.0, 1.0, 0, 235, 216, 52, 0 , 0, 0, 0)
