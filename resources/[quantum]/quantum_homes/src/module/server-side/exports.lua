...
local permission = {
    ['apartament'] = function(user_id, homeName)
        homeName = homeName..'%'
        local homeConsult = quantum.query('quantum_homes/apartamentPermissions', { home = homeName, user_id = user_id })[1]
        if (homeConsult) then
            return true
        end
        return false
...
local checkHomeOwner = function(user_id, prompt)
    ...
    serverNotify(quantum.getUserSource(user_id), 'Você não é o <b>proprietário</b> desta residência.')
    return false
end
exports('checkHomeOwner', checkHomeOwner)
...
generateApartamentName = function(name)
    ...
    repeat
        ...
        local existingApartament = quantum.query('quantum_homes/selectHomes', { home = apartamentName })[1]
...
homesAdd = function(source)
    ...
    if (user_id) then
        local prompt = quantum.prompt(source, { 'Nome da Residência', 'Passaporte' })
        ...
        if (nUser ~= user_id and nIdentity) then  
            ...
            if (response) then
                local homesCount = quantum.query('quantum_homes/countPermissions', { home = homeName })[1]
                ...
                local userConsult = quantum.query('quantum_homes/homePermissions', { home = homeName, user_id = nUser })[1]
                if (userConsult) then
                    ...
                    quantum.execute('quantum_homes/newPermissions', { user_id = nUser, home = homeName, home_owner = 0, tax = 0, garages = 0, configs = json.encode(table), vip = 0 })
                    quantum.webhook('addHouse', '```prolog\n[Quantum - Homes]\n[ACTION]: (ADD RESIDENT)\n[USER]: '..user_id..'\n[ADD]: '..nUser..'\n[HOME]: '..homeName:upper()..'\n[TYPE]: '..homeType..'\n[TABLE]: '..json.encode(homeConfig, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                    ...
                    if (homeConsult.garages == 1) then
                        local gar = quantum.query('quantum_homes/getGarage', { home = homeName })[1]
			            if (gar) then
                            local blip = json.decode(gar.blip)
				            local spawn = json.decode(gar.spawn)
                            TriggerEvent('quantum_homes:addGarage', nSource, homeName, blip, spawn)
                        end
                    end
                end
            end
    ...
    if (user_id) then
        ...
        if (nUser ~= user_id and nIdentity) then  
            ...
            if (response) then
                local userConsult = quantum.query('quantum_homes/homePermissions', { home = homeName, user_id = nUser })[1]
                if (userConsult) then
                    quantum.execute('quantum_homes/removePermissions', { home = homeName, user_id = nUser })
                    serverNotify(source, 'O <b>'..nIdentity.firstname..' '..nIdentity.lastname..'</b> foi removido de sua residência.')
                    quantum.webhook('remHouse', '```prolog\n[Quantum - Homes]\n[ACTION]: (REM RESIDENT)\n[USER]: '..user_id..'\n[REM]: '..nUser..'\n[HOME]: '..homeName:upper()..'\n[TYPE]: '..homeType..'\n[TABLE]: '..json.encode(homeConfig, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                    
                    local nSource = quantum.getUserSource(nUser)
                    if (homeConsult.garages == 1) then
                        TriggerClientEvent('quantum_garage:removeGarage', nSource, homeName)
                    end
                else
...
homesTrancar = function(source)
    ...
    if (user_id) then
        ...
        if (nearHomesType ~= 'apartament') then
            local homeConsult = quantum.query('quantum_homes/homePermissions', { home = nearHomes, user_id = user_id })[1]
            if (homeConsult) and homeConsult.home_owner == 1 then
            ...
            nearHomes = nearHomes..'%'
            local ownerConsult = quantum.query('quantum_homes/getApartamentOwner', { user_id = user_id, home = nearHomes })[1]
    ...
    if (user_id) then
        ...
        if (nUser ~= user_id and nIdentity) then  
            ...
            if (response) then
                ...
                local request = quantum.request(source, 'Você deseja transferir a residência '..homeName..' para o '..nIdentity.firstname..' '..nIdentity.lastname..'?', 30000)
                if (request) then
                    local items = exports['quantum_inventory']:getBag('homes:'..homeName)
                    quantum.execute('quantum_homes/removeResidents', { home = homeName })
                    quantum.execute('quantum_homes/updateOwner', { home = homeName, user_id = user_id, nuser_id = nUser })
                    serverNotify(source, 'Você transferiu a sua <b>residência</b> para o '..nIdentity.firstname..' '..nIdentity.lastname..'.')
                    quantum.webhook('transferHouse', '```prolog\n[Quantum - Homes]\n[ACTION]: (TRANSFER RESIDENT)\n[OLD OWNER]: '..user_id..'\n[NEW OWNER]: '..nUser..'\n[HOME]: '..homeName:upper()..'\n[TYPE]: '..homeType..'\n[TAX IN]: '..homeConsult.tax..' ('..os.date('%d/%m/%Y - %H:%M', homeConsult.tax)..')\n[ITEMS]: '..json.encode(items, { indent = true })..'\n[TABLE]: '..json.encode(homeConfig, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                end
            end
        else
...
homesVender = function(source)
    ...
    if (user_id) then
        ...
        if (response) then
            ...
            local request = quantum.request(source, 'Você deseja vender a residência '..homeName..' para a prefeitura por R$'..quantum.format(homePrice)..'?', 30000)
            if (request) then
                local items = exports['quantum_inventory']:getBag('homes:'..homeName)
                ...
                quantum.giveBankMoney(user_id, homePrice)
                --exports.qbank:extrato(user_id, 'Venda da casa', homePrice)
                TriggerClientEvent('quantum_garage:removeGarage', source, homeName)
                quantum.webhook('sellHouse', '```prolog\n[Quantum - Homes]\n[ACTION]: (SELL HOUSE)\n[USER]: '..user_id..'\n[HOME]: '..homeName:upper()..'\n[TYPE]: '..homeType..'\n[VALUE RECEIVED]: '..homePrice..'\n[ITEMS]: '..json.encode(items, { indent = true })..'\n[REMOVED HOUSE]: items and garages and upgrades\n[TABLE]: '..json.encode(homeConfig, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                serverNotify(source, 'Você vendeu a residência <b>'..homeName..'</b> para a prefeitura por R$'..quantum.format(homePrice)..'.')                    
            end
        end
    end
end
...
homesChecar = function(source)
    ...
    if (user_id) then
        ...
        local response, homeName, homeConfig, homesType, homeType, homeConsult = checkHomeOwner(user_id, prompt[1])
        if (response) then
            local query = quantum.query('quantum_homes/selectHomes', { home = homeName })
            local permList = ''
            for k, v in pairs(query) do
                local identity = quantum.getUserIdentity(v.user_id)
                if (identity) then
    ...
    if (user_id) then
        ...
        if (response) then
            ...
            if (homesTax >= 0) then
                ...
                local request = quantum.request(source, 'Deseja pagar o IPTU da residência '..homeName..' por R$'..quantum.format(taxPrice)..'?', 30000)
                if (request) then
                    if (quantum.tryFullPayment(user_id, taxPrice)) then
                        --exports.qbank:extrato(user_id, 'IPTU', -taxPrice)
                        quantum.execute('quantum_homes/updateTax', { home = homeName, tax = os.time() })
                        serverNotify(source, 'O <b>IPTU</b> da sua residência foi pago com sucesso.')
                        quantum.webhook('buyTax', '```prolog\n[Quantum - Homes]\n[ACTION]: (BUY TAX)\n[USER]: '..user_id..'\n[HOME]: '..homeName:upper()..'\n[TYPE]: '..homeType..'\n[PRICE]: '..taxPrice..'\n[TAX RENEWED]: '..homesTax..' ('..os.date('%d/%m/%Y - %H:%M', homesTax)..')\n[TABLE]: '..json.encode(homeConfig, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                    else
...
homesOther = function(source)
    local user_id = quantum.getUserId(source)
    if (user_id) then
        local result = quantum.query('quantum_homes/userList', { user_id = user_id })
        if (#result >= 1) then
            local permList = ''
            for k, v in pairs(result) do
                local taxTime = (v.tax == -1 and v.tax or parseInt(v.tax))
                permList = permList..'Residência: <b>'..v.home..'</b><br>IPTU: <b>'..quantum.getDayHours(generalConfig.lateFee * 24 * 60 * 60 - (os.time() - taxTime))..'</b>'                
        ...
        if (response) then
            ...
            for k, v in pairs(configType[homeType].interior) do
                if (k ~= '_default') then 
                    ...
                    local interiorValue = (k == homeConfig.interior) and '(Atual)<br>Decoração: <b>'..(v.decoration and 'Disponível' or 'Indisponível')..'</b>' or '<br>Valor: <b>R$'..quantum.format(v.value)..'</b><br>Decoração: <b>'..(v.decoration and 'Disponível' or 'Indisponível')..'</b>'
    ...
    if (user_id) then
        ...
        if (response) then
            ...
            if (interiorType) then
                if (quantum.checkPermissions(user_id, interiorType.perm)) then
                    if (interior ~= homeConfig.interior) then
                        local request = quantum.request(source, 'Você deseja alterar o interior da sua residência '..homeName..' para '..configInterior[interior].name..' por R$'..quantum.format(interiorType.value)..'?', 30000)
                        if (request) then
                            if (quantum.tryFullPayment(user_id, interiorType.value)) then
                                --exports.qbank:extrato(user_id, 'Upgrade Homes', -interiorType.value)
                                local items = exports['quantum_inventory']:getBag('homes:'..homeName)
                                quantum.webhook('buyInterior', '```prolog\n[Quantum - Homes]\n[ACTION]: (BUY INTERIOR)\n[USER]: '..user_id..'\n[HOME]: '..homeName:upper()..'\n[TYPE]: '..homeType..'\n[PRICE]: '..interiorType.value..'\n[OLD INTERIOR]: '..homeConfig.interior..'\n[NEW INTERIOR]: '..interior..'\n[ITEMS]: '..json.encode(items, { indent = true })..'\n[TABLE]: '..json.encode(homeConfig, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                                ...
                                quantum.execute('quantum_homes/updateConfig', { configs = json.encode(homeConfig), home = homeName })
            ...
            if (homeDecoration) then
                ...
                for k, v in pairs(homeDecoration) do
                    if (k ~= '_default' and k ~= '_perm') then 
                        local interiorValue = (k == homeConfig.decorations) and '(Atual)' or '<br>Valor: <b>R$'..quantum.format(v.value)..'</b>'
        ...
        if (response) then
            ...
            if (homeDecoration) then
                if (homeDecoration[decoration]) then
                    if (quantum.checkPermissions(user_id, homeDecoration['_perm'])) then
                        if (decoration ~= homeConfig.decorations) then
                            local request = quantum.request(source, 'Você deseja alterar o interior da sua residência '..homeName..' para '..homeDecoration[decoration].name..' por R$'..quantum.format(homeDecoration[decoration].value)..'?', 30000)
                            if (request) then
                                if (quantum.tryFullPayment(user_id, homeDecoration[decoration].value)) then
                                    --exports.qbank:extrato(user_id, 'Upgrade Homes', -homeDecoration[decoration].value)
                                    local items = exports['quantum_inventory']:getBag('homes:'..homeName)
                                    quantum.webhook('buyDecoration', '```prolog\n[Quantum - Homes]\n[ACTION]: (BUY DECORATION)\n[USER]: '..user_id..'\n[HOME]: '..homeName:upper()..'\n[TYPE]: '..homeType..'\n[PRICE]: '..homeDecoration[decoration].value..'\n[OLD DECORATION]: '..homeConfig.decorations..'\n[NEW DECORATION]: '..decoration..'\n[ITEMS]: '..json.encode(items, { indent = true })..'\n[TABLE]: '..json.encode(homeConfig, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
...
homesBau = function(source)
    ...
    if (user_id) then
        ...
        if (response) then
            ...
            if (quantum.checkPermissions(user_id, chest.perm)) then
                local _upgrade = quantum.prompt(source, { 'Aumentar o baú (Atual: '..homeConfig.chest..' | Min: '..chest.min..'kg/Máx: '..chest.max..'kg)' })[1]
                if (_upgrade) then
                    ...
                    if (_upgrade <= chest.max) then
                        if (_upgrade ~= homeConfig.chest and _upgrade > homeConfig.chest) then
                            ...
                            local request = quantum.request(source, 'Você deseja pagar R$'..quantum.format(price)..', para aumentar o baú de sua residência '..homeName..'?', 30000)
                            if (request) then
                                if (quantum.tryFullPayment(user_id, price)) then
                                    --exports.qbank:extrato(user_id, 'Upgrade Homes', -price)
                                    local items = exports['quantum_inventory']:getBag('homes:'..homeName)
                                    quantum.webhook('buyChest', '```prolog\n[Quantum - Homes]\n[ACTION]: (BUY CHEST)\n[USER]: '..user_id..'\n[HOME]: '..homeName:upper()..'\n[TYPE]: '..homeType..'\n[PRICE]: '..price..'\n[OLD CHEST]: '..homeConfig.chest..'kg\n[NEW CHEST]: '.._upgrade..'kg\n[ITEMS]: '..json.encode(items, { indent = true })..'\n[TABLE]: '..json.encode(homeConfig, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                ...
                if (homeConsult.garages == 0) then
                    if (#(GetEntityCoords(GetPlayerPed(source)) - homes.coord) <= 15.0) then
                        ...
                        if (finish and coords.spawn and coords.blip) then
                            ...
                            local request = quantum.request(source, 'Você deseja pagar R$'..quantum.format(price)..', para adicionar uma garagem em sua residência '..homeName..'?', 30000)
                            if (request) then
                                if (quantum.tryFullPayment(user_id, price)) then
                                    --exports.qbank:extrato(user_id, 'Upgrade Homes', -price)
                                    quantum.execute('quantum_homes/updateGarages', { home = homeName, garages = 1 })
                                    quantum.execute('quantum_homes/addGarage', { home = homeName, blip = json.encode(coords.blip), spawn = json.encode(coords.spawn) })
                                    serverNotify(source, 'Você comprou uma garagem para a sua residência <b>'..homeName..'</b>.')
                                    TriggerEvent('quantum_homes:addGarage', source, homeName, coords.blip, coords.spawn)
                                    quantum.webhook('buyGarage', '```prolog\n[Quantum - Homes]\n[ACTION]: (BUY GARAGE)\n[USER]: '..user_id..'\n[HOME]: '..homeName:upper()..'\n[TYPE]: '..homeType..'\n[PRICE]: '..price..'\n[COORDS]: '..json.encode({ blip = coords.blip, spawn = coords.spawn }, { indent = true })..'\n[TABLE]: '..json.encode(homeConfig, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                                else
...
local _homesLacrar = {
    ['near'] = function(source, nearHomes, inside, homeName)
        if (nearHomes) then
            ...
            local items = exports['quantum_inventory']:getBag('homes:'..nearHomes)
            quantum.webhook('sealHouse', '```prolog\n[Quantum - Homes]\n[ACTION]: (SEAL HOUSE)\n[OFFICER]: '..quantum.getUserId(source)..'\n[HOME]: '..nearHomes:upper()..'\n[ITEMS]: '..json.encode(items, { indent = true })..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
        end
    end,
    ['inside'] = function(source, nearHomes, inside, homeName)
        if (inside) then
...
homesLacrar = function(source)
    ...
    if (user_id) then
        ...
        local nearHomesType = (homes and homes.type or false)
        if (nearHomesType ~= 'apartament') then
            local homeConsult = quantum.query('quantum_homes/getHomeOwner', { home = nearHomes })[1]
            if (homeConsult) then
                local inside, homeName = insideHome(source)
            ...
            nearHomes = nearHomes..'%'
            local ownerConsult = quantum.query('quantum_homes/getApartamentOwnerWithoutUser', { home = nearHomes })[1]
            if (ownerConsult) then
                local inside, homeName = insideHome(source)
