local FormattedToken = "Bot " .. DiscordConfig.Bot_Token
...
Citizen.CreateThread(function()
	if (GetCurrentResourceName() ~= "discord") then 
		--StopResource(GetCurrentResourceName());
		-- print("[" .. GetCurrentResourceName() .. "] " .. "IMPORTANT: This resource must be named discord for it to work properly with other scripts...");
	end
	-- print("[discord] For support, make sure to join Badger's official Discord server: discord.gg/WjB5VFz");
...
function sendDebugMessage(msg)
	-- print("^1[^5discord^1] ^3" .. msg);
end

tracked = {}

RegisterNetEvent('discord:PlayerLoaded')
AddEventHandler('discord:PlayerLoaded', function()
	if (GetCurrentResourceName() ~= "discord") then 
	end
...
card = '{"type":"AdaptiveCard","$schema":"http://adaptivecards.io/schemas/adaptive-card.json","version":"1.4","body":[{"type":"Image","url":"' .. DiscordConfig.Splash.Header_IMG .. '","horizontalAlignment":"Center"},{"type":"Container","items":[{"type":"TextBlock","text":"discord","wrap":true,"fontType":"Default","size":"ExtraLarge","weight":"Bolder","color":"Light","horizontalAlignment":"Center"},{"type":"TextBlock","text":"' .. DiscordConfig.Splash.Heading1 .. '","wrap":true,"size":"Large","weight":"Bolder","color":"Light","horizontalAlignment":"Center"},{"type":"TextBlock","text":"' .. DiscordConfig.Splash.Heading2 .. '","wrap":true,"color":"Light","size":"Medium","horizontalAlignment":"Center"},{"type":"ColumnSet","height":"stretch","minHeight":"100px","bleed":true,"horizontalAlignment":"Center","columns":[{"type":"Column","width":"stretch","items":[{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Discord","url":"' .. DiscordConfig.Splash.Discord_Link .. '","style":"positive"}],"horizontalAlignment":"Right"}],"height":"stretch"},{"type":"Column","width":"stretch","items":[{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Website","style":"positive","url":"' .. DiscordConfig.Splash.Website_Link .. '"}],"horizontalAlignment":"Left"}]}]},{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Click to join Badger\'s Discord","style":"destructive","iconUrl":"https://i.gyazo.com/0904b936e8e30d0104dec44924bd2294.gif","url":"https://discord.com/invite/WjB5VFz"}],"horizontalAlignment":"Center"}],"style":"default","bleed":true,"height":"stretch"},{"type":"Image","url":"https://i.gyazo.com/7e896862b14be754ae8bad90b664a350.png","selectAction":{"type":"Action.OpenUrl","url":"https://zap-hosting.com/badger"},"horizontalAlignment":"Center"}]}'
if DiscordConfig.Splash.Enabled then 
	AddEventHandler('playerConnecting', function(name, setKickReason, deferrals) 
		...
		while not toEnd do 
			...
			Wait((1000))
			count = count + 1;
			if count == DiscordConfig.Splash.Wait then 
				toEnd = true;
			end
		end
		deferrals.done();
...
function GetGuildId (guildName)
  local result = tostring(DiscordConfig.Guild_ID)
  if guildName and DiscordConfig.Guilds[guildName] then
    result = tostring(DiscordConfig.Guilds[guildName])
  end
  return result
end
...
function FetchRoleID(roleID2Check, guild --[[optional]])
    ...
    local rolesListFromConfig = DiscordConfig.RoleList
    if rolesListFromConfig[roleID2Check] then
      -- It's a named role in the DiscordConfig. Here you go!
    ...
    if GetGuildId(guild) ~= tostring(DiscordConfig.Guild_ID) then
    ...
    if (DiscordConfig.Multiguild and not roleFound) then
      for guildName, guildID in pairs(DiscordConfig.Guilds) do
...
function IsDiscordEmailVerified(user) 
    ...
    if discordId then 
        ...
        if member.code == 200 then
        	...
        	sendDebugMessage("[discord] ERROR: Code 200 was not reached. DETAILS: " .. error_codes_defined[member.code]);
        end
    end
    return isVerified;
end
  ...
  local guildId = GetGuildId(guild)
	...
	if guild.code == 200 then
		...
		if (data.icon:sub(1, 1) and data.icon:sub(2, 2) == "_") then 
			-- It's a gif 
			return 'https://cdn.discordapp.com/icons/' .. DiscordConfig.Guild_ID .. "/" .. data.icon .. ".gif";
		else 
			-- Image 
			return 'https://cdn.discordapp.com/icons/' .. DiscordConfig.Guild_ID .. "/" .. data.icon .. ".png";
		end 
	else
		sendDebugMessage("[discord] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data or guild.code)) 
	end
	return nil;
end
...
function GetGuildSplash(guild --[[optional]])
  local guildId = GetGuildId(guild)
	...
	if guild.code == 200 then
		local data = json.decode(guild.data)
		-- Image 
		return 'https://cdn.discordapp.com/splashes/' .. DiscordConfig.Guild_ID .. "/" .. data.icon .. ".png";
	else
		sendDebugMessage("[discord] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data or guild.code)) 
	end
	return nil;
...
function GetDiscordAvatar(user) 
	...
	if discordId then 
		...
		sendDebugMessage("[discord] ERROR: Discord ID was not found...")
	end
    return imgURL;
end
...
function GetDiscordRoles(user, guild --[[optional]])
  ...
  local roles = nil
	...
	if discordId then
		...
		if guild or DiscordConfig.Multiguild == false then
			return roles
		end
		...
		for _,id in pairs(DiscordConfig.Guilds) do
		...
		sendDebugMessage("[discord] ERROR: Discord was not connected to user's Fivem account...")
		return false
	end
	return false
end
...
function GetUserRolesInGuild (user, guild)
	...
	if not user then
		sendDebugMessage("[discord] ERROR: GetUserRolesInGuild requires discord ID")
		return false
	end
	if not guild then
		sendDebugMessage("[discord] ERROR: GetUserRolesInGuild requires guild ID")
		return false
	end

	-- Check for cached roles
	if DiscordConfig.CacheDiscordRoles and recent_role_cache[user] and recent_role_cache[user][guild] then
	...
	if member.code == 200 then
		local data = json.decode(member.data)
		local roles = data.roles
		if DiscordConfig.CacheDiscordRoles then
			recent_role_cache[user] = recent_role_cache[user] or {}
			recent_role_cache[user][guild] = roles
			Citizen.SetTimeout(((DiscordConfig.CacheDiscordRolesTime or 60)*1000), function() recent_role_cache[user][guild] = nil end)
		end
		return roles
	else
		sendDebugMessage("[discord] ERROR: Code 200 was not reached... Returning false. [Member Data NOT FOUND] DETAILS: " .. error_codes_defined[member.code])
		return false
	end
end
  ...
  local guildId = GetGuildId(guild)
	...
	if discordId then
		...
		if member.code == 200 then
			...
			sendDebugMessage("[discord] ERROR: Code 200 was not reached. Error Code: " .. error_codes_defined[member.code])
			return nil;
		end
	else
		sendDebugMessage("[discord] ERROR: Discord was not connected to user's Fivem account...")
...
function SetNickname(user, nickname, reason)
	...
	if discordId then
		...
		local endpoint = ("guilds/%s/members/%s"):format(DiscordConfig.Guild_ID, discordId)
...
Citizen.CreateThread(function()
  local mguild = DiscordRequest("GET", "guilds/"..DiscordConfig.Guild_ID, {})
  if mguild.code == 200 then
    local data = json.decode(mguild.data)
    sendDebugMessage("[discord] Successful connection to Guild : "..data.name.." ("..data.id..")")
  else
    sendDebugMessage("[discord] An error occured, please check your config and ensure everything is correct. Error: "..(mguild.data and json.decode(mguild.data) or mguild.code)) 
  end
  if (DiscordConfig.Multiguild) then 
    for _,guildID in pairs(DiscordConfig.Guilds) do
      local guild = DiscordRequest("GET", "guilds/"..guildID, {})
      if guild.code == 200 then
        local data = json.decode(guild.data)
        sendDebugMessage("[discord] Successful connection to Guild : "..data.name.." ("..data.id..")")
      else
        sendDebugMessage("[discord] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data and json.decode(guild.data) or guild.code)) 
      end
    end
  end
end)
