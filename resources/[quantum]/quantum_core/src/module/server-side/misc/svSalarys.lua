...
local salarys = {
    ...
    -- STAFF
    ----------------------------------
    { name = 'New Valley CEO', value = 300000, perm = '@Staff.CEO' },
    { name = 'New Valley COO', value = 25000, perm = '@Staff.COO' },
    { name = 'New Valley Manager', value = 10000, perm = '@Staff.Manager' },
    { name = 'New Valley Adm', value = 5000, perm = '@Staff.Administrador' },
    ----------------------------------
    -- VIPS
    ----------------------------------
    ...
    { name = '[VIP] quantum', value = 7500, perm = '@Vips.quantum' },
    ----------------------------------
    -- Jurídico
    ----------------------------------
...
giveSalary = function(source, user_id)
    if (source and user_id) then
        for k, v in pairs(salarys) do
            local value = v.value
            if (quantum.hasPermission(user_id, v.perm)) then
                if (string.find(v.perm, '@Policia') or string.find(v.perm, '@Deic')) and quantum.hasPermission(user_id, 'vippm.permissao') then value = parseInt(value + 5000); end;
                quantum.giveBankMoney(user_id, value)
                --exports.qbank:extrato(user_id, 'Salário ', value)

                TriggerClientEvent('quantum_sound:source', source, 'coins', 0.5)
                TriggerClientEvent('notify', source, 'Salário', 'Olá! Seu salário ('..v.name..') de <b>R$'..quantum.format(value)..'</b> foi depositado em sua conta bancária.')
            end
        end
    end
end
