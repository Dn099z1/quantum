...
RegisterNetEvent('quantum:playerExit', function(user_id, reason, drawCDS)
	if (drawCDS) then
		if reason and #(pExitCDS - drawCDS) <= 500 then
			local draw = true
			local str = '~b~[quantum - QUIT]~w~\nPASSAPORTE: ~b~'..user_id..'~w~\nMOTIVO: ~b~'..reason..'~w~'
...
-- COMMANDS
---------------------------------------
RegisterNetEvent('quantum_commands_police:clothes', function(part)
    local ped = PlayerPedId()
...
RegisterNetEvent('quantum_core:spikeThread', spikeThread)
...
RegisterNetEvent('vTow',function()
	...
	if IsVehicleModel(vehicle,GetHashKey('flatbed3')) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
		CommandsData['mec:towed'] = quantum.getNearestVehicle(7)
...
-- CAR COLOR
---------------------------------------
RegisterNetEvent('quantum_core:carcolor', function(veh, r, g, b, prim)
...
-- UNCUFF
---------------------------------------
RegisterNetEvent('quantum_core:uncuff', function()
	local ped = PlayerPedId()
	quantum._setHandcuffed(false)
...
-- DELNPCS
---------------------------------------
RegisterNetEvent('quantum_core:delnpcs', function()
...
-- TUNING
---------------------------------------
RegisterNetEvent('quantum_core:tuning', function()
	local ped = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(ped)
	if (IsEntityAVehicle(vehicle)) then
		SetVehicleModKit(vehicle,0)
		--SetVehicleWheelType(vehicle,7)
		SetVehicleCustomPrimaryColour(vehicle, 216, 179, 13)    
    	...
    	SetVehicleNumberPlateText(vehicle, 'quantum')
...
RegisterNetEvent('quantum_core:debug', function()
...
RegisterNetEvent('quantum_core:terremoto', function()
...
-- RICH PRESENCE
---------------------------------------
RegisterNetEvent('quantum_core:discord', function(status)
    SetDiscordAppId(1164230990399549440)
    SetDiscordRichPresenceAsset('valley_sem_fundo')
	SetDiscordRichPresenceAssetSmall('valley_sem_fundo')
	SetDiscordRichPresenceAssetSmallText('New Valley')
    SetRichPresence(status)
    SetDiscordRichPresenceAction(0, 'JOGAR', 'https://newvalley.online')
    SetDiscordRichPresenceAction(1, 'DISCORD', 'https://discord.gg/newvalleyrp')
end)
