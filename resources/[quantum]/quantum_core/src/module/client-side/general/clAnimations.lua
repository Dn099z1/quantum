...
RegisterNetEvent('quantum_animations:setAnim', function(anim)
    ...
    if (not IsPedInAnyVehicle(ped) and not emote.carros) then       
        ...
        if emote.pos then
            ...
            if (emoteDict) then quantum._playAnim(emote.andar, {{ emote.dict, emote.anim }}, emote.loop); end;
            if (type(emote.prop) == 'table') then
                quantum.CarregarObjeto('', '', emote.prop[1], emote.flag, emote.hand[1], emote.pos[1], emote.pos[2], emote.pos[3], emote.pos[4], emote.pos[5], emote.pos[6])
                quantum.CarregarObjeto('', '', emote.prop[2], emote.flag, emote.hand[2], emote.pos2[1], emote.pos[2], emote.pos2[3], emote.pos2[4], emote.pos2[5], emote.pos2[6])
            else
                quantum.CarregarObjeto('', '', emote.prop, emote.flag, emote.hand, emote.pos[1], emote.pos[2], emote.pos[3], emote.pos[4], emote.pos[5], emote.pos[6])
            end
        elseif (emote.prop) then
            quantum.CarregarObjeto(emote.dict, emote.anim, emote.prop, emote.flag, emote.hand)
        elseif (emote.dict) then
            quantum._playAnim(emote.andar, {{emote.dict,emote.anim}}, emote.loop)
        else
            quantum._playAnim(false, { task = emote.anim }, false)
        end
    else
...
RegisterNetEvent('quantum_animations:cancelSharedAnimation', function()
    local ped = PlayerPedId()
    ClearPedTasks(ped); DetachEntity(ped, true, false);
    if (sharedAnimation) then
        TriggerServerEvent('quantum_animation:sharedServer', sharedAnimation)
        sharedAnimation = nil
    end
end)

RegisterNetEvent('quantum_animation:sharedClearAnimation', function()
    local ped = PlayerPedId()
    quantum.DeletarObjeto()
    ClearPedTasks(ped); DetachEntity(ped, true, false);
end)

RegisterNetEvent('quantum_animations:setAnimShared', function(anim, target)
    if (LocalPlayer.state.BlockTasks and LocalPlayer.state.Handcuff and IsPedFalling(PlayerPedId())) then return; end;
...
RegisterNetEvent('quantum_animations:setAnimShared2', function(anim, target)   
    if (LocalPlayer.state.BlockTasks and LocalPlayer.state.Handcuff and IsPedFalling(PlayerPedId())) then return; end;
    sharedAnimation = target
    local ped = PlayerPedId()
    local emote = configAnimations.shared[anim]
