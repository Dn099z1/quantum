...
RegisterCommand('attachs',function(source, args)
	...
	if (vSERVER.checkAttachs({ 'attachs.permissao', 'attachs2.permissao', '@Vips.quantum' })) then
		if not (args[1]) then
...
RegisterCommand('attachs2',function(source, args)
	local ped = PlayerPedId()
	if (vSERVER.checkAttachs({ 'attachs2.permissao', '@Vips.quantum' }, true)) then
		if not (args[1]) then
...
RegisterCommand('wcolor',function(source, args)
	...
	if (tinta >= 0) and vSERVER.checkColor({ 'attachs2.permissao', '@Vips.quantum' }) then
		local ped = PlayerPedId()
