Citizen.CreateThread(function()
    ...
    AddTextEntry('FE_THDR_GTAO', 'quantum')
...
Citizen.CreateThread(function()
		SetPedDensityMultiplierThisFrame(0.15) -- set npc/ai peds density to 0
		SetRandomVehicleDensityMultiplierThisFrame(0.15) -- set random vehicles (car scenarios / cars driving off from a parking spot etc.) to 0
		SetScenarioPedDensityMultiplierThisFrame(0.15, 0.15) -- set random npc/ai peds or scenario peds to 0
	...
	while (true) do
		...
		if (not Dispatch and IsPedArmed(ped, 4)) then
			...
			if (IsPedShooting(ped)) and not (BlacklistDispatch[GetSelectedPedWeapon(ped)]) then
				Dispatch = true
				TriggerServerEvent('quantum_core:shoting', GetEntityCoords(ped))
...
local shot = {}
RegisterNetEvent('quantum_core:shotingBlip', function(coord, user_id)
