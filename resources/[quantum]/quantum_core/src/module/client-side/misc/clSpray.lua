...
RegisterNetEvent('quantum_spray:setSprays', function(s)
    SPRAYS = s
end)
...
RegisterNetEvent('quantum_spray:removeClosestSpray', function()
    local ped = PlayerPedId()
    local coords = GetEntityCoords(ped)
    ...
    if (closestSprayLoc) then
        ...
        function()
            ...
            TriggerServerEvent('quantum_spray:remove', closestSprayLoc)
        end, 
        function()
            RemoveSprayRemoveProp(ragProp)
        end)
