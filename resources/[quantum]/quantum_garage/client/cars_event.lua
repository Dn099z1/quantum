...
AddEventHandler('gameEventTriggered', function(event, args)
    if (event == 'CEventNetworkPlayerEnteredVehicle') then
        ...
        if (id == PlayerId()) then            
            ...
            TriggerEvent('quantum_core:spikeThread')
            ...
            if (blackVehicles[model]) then 
                if (blackVehicles[model][2]) then TriggerServerEvent('quantum_anticheat', 'vehicles', blackVehicles[model][1]);  end;
            end
            drivingVehicle(model)

            local class = GetVehicleClass(vehicle)
            if (class ~= 14 and class ~= 13) then  end;
        ...
        while (inVehicle) do
            ...
            if (inVehicle) then                
                ...
                if (GetPedInVehicleSeat(vehicle, -1) == ped or GetPedInVehicleSeat(vehicle, 0) == ped) then
                    ...
                    -- P1 PERMITIR ATIRAR NO CARRO
                    -- Removido o código que desabilitava o disparo no banco do motorista
                    -- if (GetPedInVehicleSeat(vehicle, -1) == ped) then
