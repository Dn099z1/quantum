...
quantum.prepare('quantum_garage/getVehicles', 'select * from user_vehicles where user_id = @user_id')
quantum.prepare('quantum_garage/getVehiclesWithVeh', 'select * from user_vehicles where user_id = @user_id and vehicle = @vehicle')
...
quantum.prepare('quantum_garage/setDetained', 'update user_vehicles set detained = @detained where user_id = @user_id and vehicle = @vehicle')
quantum.prepare('quantum_garage/setIPVA', 'update user_vehicles set ipva = @ipva where user_id = @user_id and vehicle = @vehicle')
quantum.prepare('quantum_garage/updateState', 'update user_vehicles set state = @state where user_id = @user_id and vehicle = @vehicle')
quantum.prepare('quantum_garage/getVehiclePlate', 'select plate from user_vehicles where user_id = @user_id and vehicle = @vehicle')
quantum.prepare('quantum_garage/delVehicle', 'delete from user_vehicles where user_id = @user_id and vehicle = @vehicle')
quantum.prepare('quantum_garage/setRented', 'update user_vehicles set rented = @rented where user_id = @user_id and vehicle = @vehicle')
quantum.prepare('quantum_garage/getVehicleWithPlate', 'select user_id, vehicle from user_vehicles where plate = @plate')
quantum.prepare('quantum_garage/updatePlate', 'update user_vehicles set plate = @plate where user_id = @user_id and vehicle = @vehicle')
quantum.prepare('quantum_garage/getVehicleOwn', 'select * from user_vehicles where user_id = @user_id and service = 0')
quantum.prepare('quantum_garage/getGarages', 'select garages from users where id = @id')
quantum.prepare('quantum_garage/getVehByPlate', 'select * from user_vehicles where plate = @plate')
quantum.prepare('quantum_garage/getVehByChassis', 'select * from user_vehicles where chassis = @chassis')
quantum.prepare('quantum_garage/getRented', 'select user_id, vehicle, rented from user_vehicles where rented != ""')
...
Citizen.CreateThread(function()
    Citizen.Wait(1000)
	for user_id, source in pairs(quantum.getUsers()) do
		local myHomes = quantum.query('quantum_homes/userHomesList', { user_id = user_id })
        for k,v in ipairs(myHomes) do
            local gar = quantum.query('quantum_homes/getGarage', { home = v.home })[1]
            if (gar) then
...
getCarTrunkWeight = function(user_id, vehicle, capacity)
    local bag = exports['quantum_inventory']:getBag('trunk:'..vehicle..':'..user_id)
    local weight = 0
...
local vehicles = {
    ['service'] = function(source, user_id, myVehicles, gInfo)
        for _, v in pairs(gInfo.vehicles) do
            ...
            local data = quantum.query('quantum_garage/getVehiclesWithVeh', { user_id = user_id, vehicle = v })
            if (data) then
    ...
    ['normal'] = function(source, user_id, myVehicles, gInfo)
        local vehicles = quantum.query('quantum_garage/getVehicles', { user_id = user_id })
...
local setDetained = function(user_id, vehicle, number)
    quantum.execute('quantum_garage/setDetained', { user_id = user_id, vehicle = vehicle, detained = number })
end
...
local vehicleDetained = {
    ...
    [1] = function(source, user_id, vehicle)
        ...
        local request = quantum.request(source, 'Veículo na detenção, deseja acionar o seguro pagando R$'..quantum.format(value)..'?', 60000)
        if (request) then
            if (quantum.tryFullPayment(user_id, value)) then
                --exports.qbank:extrato(user_id, 'Veículo detido', -value)
    ...
    [2] = function(source, user_id, vehicle)
        ...
        local request = quantum.request(source, 'Veículo na retenção, deseja acionar o seguro pagando R$'..quantum.format(value)..'?', 60000)
        if (request) then
            if (quantum.tryFullPayment(user_id, value)) then
                setDetained(user_id, vehicle, 0)
                --exports.qbank:extrato(user_id, 'Veículo retido', -value)
                TriggerClientEvent('notify', source, 'Garagem', 'O <b>pagamento</b> foi efetuado com sucesso.')
                return true
            else
                TriggerClientEvent('notify', source, 'Garagem', 'Você não possui <b>dinheiro</b> suficiente para este pagamento.')
        ...
        if vehicle and (not spawnTasks[source]) then
            ...
            if (user_id) then
                if (not findVehicle(user_id, vehicle)) then
                    local veh = quantum.query('quantum_garage/getVehiclesWithVeh', { user_id = user_id, vehicle = vehicle })[1]
                    if (not veh) then
                        addVehicle(user_id, vehicle, 1)
                        veh = quantum.query('quantum_garage/getVehiclesWithVeh', { user_id = user_id, vehicle = vehicle })[1]
                    end
                    ...
                    if (veh.service == 0) then
                        ...
                        if (os.time() >= parseInt(veh.ipva + (config.ipvaDays * 24 * 60 * 60))) then
                            ...
                            local request = quantum.request(source, 'Deseja pagar o Vehicle Tax do veículo '..vehicleName(vehicle)..' por R$'..quantum.format(priceTax)..'?', 60000)
                            if (request) then
                                if (quantum.tryFullPayment(user_id, priceTax)) then
                                    quantum.execute('quantum_garage/setIPVA', { user_id = user_id, vehicle = vehicle, ipva = os.time() })
                                    TriggerClientEvent('notify', source, 'Garagem', 'O <b>pagamento</b> foi efetuado com sucesso.')
                                else
                                    TriggerClientEvent('notify', source, 'Garagem', 'Você não possui <b>dinheiro</b> suficiente para este pagamento.')
                                end
                    ...
                    if (loadModel) then
                        ...
                        if (slot) then
                            ...
                            local vehHandle = NetworkGetEntityFromNetworkId(vehNet)
                        --    exports.quantum_garage:lockVehicleState()
                        TriggerClientEvent('quantum_garage:lockVehicleState',source)
...
srv.tryDelete = function(vehnet, state)
	if vehnet then
		...
		if (vehState.user_id) then
			local player = quantum.getUserSource(vehState.user_id)
            TriggerClientEvent('quantum_garage:freeVehicleState',player)
            TriggerClientEvent('notify', source, 'Garagem', 'O Veiculo foi guardado!')
			if player then vCLIENT.removeGPSVehicle(player, vehState.model); end;
			if state and (not vehState.spawning) then
				local vehicle = quantum.query('quantum_garage/getVehiclesWithVeh', { user_id = vehState.user_id, vehicle = vehState.model })
				if (vehicle[1]) then
					quantum.execute('quantum_garage/updateState', { user_id = vehState.user_id, vehicle = vehState.model, state = json.encode(state) })
				end
			end
		end
	end
...
srv.vehicleLock = function()
    ...
    local user_id = quantum.getUserId(source)
    if (user_id) then
        local cveh, vnetid, placa, vname, lock, banned = quantumClient.vehList(source, 7.5)
        if (vnetid and vname) then
            ...
            if (user_id == vehState.user_id) or (user_id == carKeys[vnetid]) then
                ...
                if (not quantumClient.isInVehicle(source)) then quantumClient._playAnim(source, true, {{ 'anim@mp_player_intmenu@key_fob@', 'fob_click' }}, false); end;
                TriggerClientEvent('quantum_sound:vehicle', -1, vnetid, 15.0, 'lock', 0.01)
                if (lock == 1) then
...
RegisterCommand('dvarea', function(source)
    ...
    local identity = quantum.getUserIdentity(user_id)
    if (user_id) and quantum.hasPermission(user_id, 'staff.permissao') then
        local prompt = quantum.prompt(source, { 'Distância' })
        if (prompt[1]) then
            prompt[1] = parseInt(prompt[1])
            local getVehicles = quantumClient.getNearestVehicles(source, prompt[1])
            local deleteCount = 0
...
RegisterCommand('addcar', function(source)
    ...
    if (user_id) and quantum.hasPermission(user_id, '+Staff.COO') then
        local prompt = quantum.prompt(source, { 'Passaporte', 'Veículo' })
        if (prompt) then
...
RegisterCommand('car', function(source, args)
    ...
    if (user_id) and quantum.hasPermission(user_id, '+Staff.Administrador') then
        if (args[1]) then
            ...
            TriggerEvent('registerVehicle',spawn)
            ...
            if (loadModel) then
                ...
                local userVehicle = (quantum.query('quantum_garage/getVehiclesWithVeh', { user_id = user_id, vehicle = spawn })[1] or {})
                local plate = (userVehicle.plate or identity.registration)

                -- Using only client spawn
...
RegisterNetEvent('quantum_interactions:carTrancar', function()
    local source = source
	local user_id = quantum.getUserId(source)
	if (user_id) and quantum.hasPermission(user_id, 'policia.permissao') then
        if (quantumClient.isInVehicle(source)) then
            local vehicle, vnetid = quantumClient.vehList(source, -1)
            if (vnetid) then
...
RegisterNetEvent('quantum_interactions:carAncorar', function()
    local source = source
    if (quantumClient.isInVehicle(source)) then
        local vehicle = quantumClient.vehList(source, -1)
...
local keys = {
    ['add'] = function(source, nSource, vnetid, vname, nIdentity, identity, nuserId, user_id)
        local request = quantum.request(source, 'Dar a chave reserva do veículo '..vehicleName(vname)..' para '..nIdentity.firstname..' '..nIdentity.lastname..'?', 60000)
        if (request) then
            carKeys[vnetid] = nuserId
            quantumClient._playAnim(source, true, {{ 'mp_common', 'givetake1_a' }}, false)
            quantumClient._playAnim(nSource, true, {{ 'mp_common', 'givetake1_a' }}, false)
...
RegisterNetEvent('quantum_interactions:carKeys', function(value)
    local source = source
    local user_id = quantum.getUserId(source)
    if (user_id) then
        local identity = quantum.getUserIdentity(user_id)
        ...
        if (nPlayer) then
            ...
            local vehicle, vnetid, placa, vname = quantumClient.vehList(source, 3.0)
            if (vnetid and vname) then
                local vehState = srv.getVehicleData(vnetid)
...
RegisterNetEvent('quantum_interactions:carVehs', function()
    local source = source
    local user_id = quantum.getUserId(source)
    if (user_id) then
        local prompt = quantum.prompt(source, { 'Veículo que você deseja vender', 'Valor do veículo', 'Passaporte do jogador que irá receber o veículo' })
        if (prompt) then
            local model = prompt[1]
            local vname = vehicleName(model)

            local myVehicle = quantum.query('quantum_garage/getVehiclesWithVeh', { user_id = user_id, vehicle = model })[1]
            if (myVehicle) then
                if ((vehicleType(model) == 'work') or (vehicleType(model) == 'exclusive') or (vehicleType(model) == 'vip') or (myVehicle.rented ~= '')) then
                    TriggerClientEvent('notify', source, 'Venda de veículos', 'Este <b>veículo</b> não pode ser vendido.')
                    return
                end
                ...
                if (nSource) then
                    ...
                    if (price > 0) then
                        if (quantum.request(source, 'Deseja vender um '..vname..' para '..nIdentity.firstname..' '..nIdentity.lastname..' por R$'..quantum.format(price)..' ?', 60000)) then
                            if (quantum.request(nSource, 'Aceita comprar um '..vname..' de '..identity.firstname..' '..identity.lastname..' por R$'..quantum.format(price)..'?', 60000)) then
                                local userVehicle = quantum.query('quantum_garage/getVehiclesWithVeh', { user_id = nUser, vehicle = model })[1]
                                if (userVehicle) then
                                    TriggerClientEvent('notify', source, 'Venda de veículos', 'O <b>'..nIdentity.firstname..' '..nIdentity.lastname..'</b> já possui este modelo de veículo')
                                end

                                if (quantum.tryFullPayment(nUser, price)) then
                                    --exports.qbank:extrato(nUser, 'Compra de veículo', -price)
                                    addVehicle(nUser, model, 0)
                                    delVehicle(user_id, model)

                                    TriggerClientEvent('notify', source, 'Garagem', 'Você vendeu <b>'..vname..'</b> e recebeu <b>R$'..quantum.format(price)..'</b>.')
                                    TriggerClientEvent('notify', nSource, 'Garagem', 'Você recebeu as chaves do veículo <b>'..vname..'</b> de <b>'..identity.firstname..' '..identity.lastname..'</b> e pagou <b>R$'..quantum.format(price)..'</b>.')
                                    
                                    quantumClient.playSound(source, 'Hack_Success', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS')
                                    quantumClient.playSound(nSource, 'Hack_Success', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS')
                                    
                                    quantum.giveBankMoney(user_id, price)
                                    --exports.qbank:extrato(user_id, 'Venda de veículo', price)
                                    quantum.webhook('Vehs', '```prolog\n[JOGADOR]: #'..user_id..' '..identity.firstname..' '..identity.lastname..' \n[VENDEU]: '..vehicleName(vname)..' \n[PARA]: #'..nUser..' '..nIdentity.firstname..' '..nIdentity.lastname..'\n[POR]: '..quantum.format(price)..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
                                    ...
                                    TriggerClientEvent('notify', nSource, 'Venda de veículos', 'Você não possui <b>dinheiro</b> o suficiente.')
                                    TriggerClientEvent('notify', source, 'Venda de veículos', 'O mesmo não possui <b>dinheiro</b> o suficiente.')
                                end
                            end
                        end
                    end
...
RegisterNetEvent('quantum_interactions:carPlate', function()
    local source = source
    local user_id = quantum.getUserId(source)
	if (user_id and quantum.hasPermission(user_id, 'staff.permissao') or quantum.hasPermission(user_id, 'policia.permissao') or quantum.hasPermission(user_id, 'desmanche.permissao')) then
        local vehicle, vnetid, placa, vname, lock, banned = quantumClient.vehList(source, 7.0)
        if (vnetid) then
            local vehState = srv.getVehicleData(vnetid)
            if (vehState.user_id) then
                local identity = quantum.getUserIdentity(vehState.user_id)
                if (identity) then
...
RegisterNetEvent('quantum:fixCommand', function(source)
    local source = source
    local user_id = quantum.getUserId(source)
    local identity = quantum.getUserIdentity(user_id)
    if (user_id) and quantum.hasPermission(user_id, 'staff.permissao') then
        local vehicle = quantumClient.getNearestVehicle(source, 12.0)
        if (vehicle) then
            local vehicle, vnetid, placa, vname, lock, banned = quantumClient.vehList(source, 3.0)
            if (vnetid) then
                local vehState = srv.getVehicleData(vnetid)
                TriggerClientEvent('syncreparar', -1, vnetid)
                quantum.webhook('Fix', '```prolog\n[/FIX]\n[STAFF]: #'..user_id..' '..identity.firstname..' '..identity.lastname..'\n[FIX]: '..vehState.model..'\n[DONO]: '..vehState.user_id..' \n[COORDS]: '..tostring(GetEntityCoords(GetPlayerPed(source)))..os.date('\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S')..' \r```')
            end
        end
    end
end)
...
RegisterCommand('fix',function(source, args, rawCommand)
    print('fix')
    TriggerEvent('quantum:fixCommand')
end)
...
RegisterCommand('chassis',function(source, args, rawCommand)
	...
	if (user_id) and quantum.hasPermission(user_id, 'policia.permissao') then
		if (args[1]) then
			...
			local vehQuery = quantum.query('quantum_garage/getVehByChassis', { chassis = chassis })[1]
			if (vehQuery.vehicle) then
				local identity = quantum.getUserIdentity(vehQuery.user_id)
				if (identity) then
...
RegisterNetEvent('quantum_interactions:carTrunkin', function()
    local source = source
    if (GetEntityHealth(GetPlayerPed(source)) > 100 and not quantumClient.isHandcuffed(source)) then
        TriggerClientEvent('quantum_garage:enterTrunk', source)
    end
end)
...
RegisterNetEvent('quantum_interactions:checkTrunkin', function()
    local source = source
    if (GetEntityHealth(GetPlayerPed(source)) > 100 and not quantumClient.getNoCarro(source) and not quantumClient.isHandcuffed(source)) then
        local nplayer = quantumClient.getNearestPlayer(source, 2.0)
        if (nplayer) then 
            TriggerClientEvent('notify', source, 'Garagem', 'Retirando cidadão do <b>porta-malas</b>.')
            TriggerClientEvent('quantum_garage:checkTrunk', nplayer)
        end
    end
end)
...
RegisterNetEvent('quantum_homes:addGarage', function(source, homeName, blip, spawn)
    local homes = quantum.query('quantum_homes/permissions', { home = homeName })
    for _, v in ipairs(homes) do
        local source = quantum.getUserSource(v.user_id)
...
AddEventHandler('onResourceStart', function(resourceName)
  	if (GetCurrentResourceName() == resourceName) then 
        TriggerClientEvent('quantum:clearVehicleCache', -1)

        local rentedVehs = quantum.query('quantum_garage/getRented')
        ...
        print('^3Feito Por ^8DnzxDevop^6 - Quantum Devs')
        print('^5[Quantum - Garages]^7 foram deletados '..count..' veiculos alugado.')
	end
end)
...
RegisterCommand('vehs', function(source)
    local user_id = quantum.getUserId(source)
    if (user_id) then
        local vehicle = quantum.query('quantum_garage/getVehicleOwn', { user_id = user_id })
        if (#vehicle > 0) then
            local car_names = {}
...
RegisterNetEvent('quantum_interactions:vehs', function()
    local source = source
    local user_id = quantum.getUserId(source)
    if (user_id) then
        local vehicle = quantum.query('quantum_garage/getVehicleOwn', { user_id = user_id })
...
srv.setDM = function(model,dm)
    SetEntityRoutingBucket(model,dm)
end

srv.getDMEntity = function(source)
    source = source
    local Dm = GetEntityRoutingBucket(source)
    return Dm
end