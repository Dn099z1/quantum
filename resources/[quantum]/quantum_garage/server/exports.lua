...
vehicleName = function(vehicle)
	...
	print('^5[Quantum - Garages] Vehicle ['..tostring(vehicle)..'] not found.^0')
end
...
vehHashData = function(hash, displayName)
	...
	print('^5[Quantum - Garages] Vehicle ['..hash..'/'..tostring(displayName)..'] not found.^0')
end
...
delVehicle = function(user_id, vehicle)
	quantum.execute('quantum_garage/delVehicle', { user_id = user_id, vehicle = vehicle })
end
...
setRentedVehicle = function(user_id, vehicle, days)
	...
	quantum.execute('quantum_garage/setRented', { user_id = user_id, vehicle = vehicle, rented = timing })
end
exports('setRentedVehicle', setRentedVehicle)

updateVehiclePlate = function(source, user_id, vehicle, plate)
	local query = quantum.query('quantum_garage/getVehicleWithPlate', { plate = plate })[1]
	if (not query) then
		quantum.execute('quantum_garage/updatePlate', { user_id = user_id, vehicle = vehicle, plate = plate })
...
getVehicleInfosWithPlate = function(plate)
	local vehData = quantum.query('quantum_garage/getVehByPlate', { plate = (plate or '') })[1]
